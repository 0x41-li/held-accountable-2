...
export default function Poll({ poll }) {
  ...
  const handleVote = async (optionId) => {
    setSelectedOptions(
      selectedOptions.map((o, i) => (i === curQueId ? optionId : o))
    );
  ...
  return (
    <div className="rounded-[12px] border border-secondary shadow-xs flex flex-col p-[16px] gap-[11px] md:gap-[20px] md:px-[24px] md:py-[17px] w-full bg-white">
      ...
      <div className="text-[16px] leading-[28px] font-medium pl-[15px]">
        {poll.questions[curQueId].headline && (
          <p className="font-bold">
            Breaking News - {poll.questions[curQueId].headline}
          </p>
        )}
        ...
        {poll.questions[curQueId].summary && showSummary ? (
        ...
        <div className="flex w-full flex-col pt-[12px]">
          ...
          <div className="w-full overflow-auto">
            <div className="flex w-max gap-[10px]">
              {poll.golden_insights.map((narrative, i) => (
                <Link
                  href={`/narrative-insights/${poll.id}`}
                  key={narrative.id + "_poll_link"}
                >
                  <div
