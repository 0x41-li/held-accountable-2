      ...
      <div className="relative w-full overflow-hidden">
        ...
        <div className="flex w-max animate-marquee gap-6" style={{ animationDuration: `${speed}s` }}>
          {data.concat(data).map((item, index) => (
              trend ? <p className="flex items-center gap-[4px]">{item.change >= 0 ? <Icon icon="icon-park-solid:up-one" className="text-green-400" />:<Icon icon="icon-park-solid:down-one" className="text-red-400" />} {item.symbol} {item.price.toLocaleString("en-US", {
  style: "currency",
  currency: "USD",
})}</p>
            :<div
