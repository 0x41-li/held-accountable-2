...
export default function BlogDetail() {
    ...
    useEffect(() => {
        fetch(`/api/enterprise/detail?id=${id}`).then(res => res.json()).then(data => {
    ...
    <div className='w-full h-full overflow-hidden md:rounded-tl-[40px] pt-[32px] border border-secondary flex flex-col bg-[#FCFCFD]'>
          <div className='flex px-[24px] pb-[20px] border-b border-secondary items-start'>
            <div className='flex flex-col gap-[4px] flex-1'>
              <div className='text-[30px] leading-[38px] font-semibold'>Enterprise Radar</div>
              <div className='text-[16px] leading-[24px] text-[#7C7C7C]'>Follow the most popular users and companies here.</div>
            </div>
          </div>
