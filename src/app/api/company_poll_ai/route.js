...
function getExistingArticles(url, title) {
    return db.prepare("SELECT * FROM business_news_articles WHERE url = ? OR title = ?").get(url, title) ?? []
}

function addNewsArticle(article) {
    return db.prepare("INSERT INTO business_news_articles (title, url, date, body) VALUES (?, ?, ?, ?)").run(article.title, article.url, article.date, article.body)
}
